...
## 四·补、每帧的"扇出" + 状态机 vs NN 如何用多帧

### 每帧同时喂给两路(扇出)
检测任务从队列取一帧后,**同一次循环里把这帧同时喂给环形缓冲和状态机**(逐帧无遗漏):
```
osMessageQueueGet(&sample)   取一帧
      │
      ├─▶ EI_Fall_Push(sample)      ① 存进环形(NN历史窗口),只存不判
      └─▶ Fall_Algo_Process(sample) ② 送进状态机(路径B),判并返回 status
              │ status==1(触发)
              └─▶ EI_Fall_Classify() 才对环形3秒跑一次NN
```
- 环形缓冲**天天在存**(为触发时随时有3秒可用);**NN 不是每帧跑,只在状态机触发时才跑一次**(省算力)。
- 两路都必须逐帧无遗漏:环形漏帧→窗口有洞;状态机漏帧→错过失重/冲击瞬变。

### 关键:状态机也需要很多帧,但方式和 NN 不同
两者都判不了单帧、都要很多帧,但"用多帧"的方式相反:

| | 状态机(路径B) | NN |
|---|---|---|
| 处理方式 | **一帧一帧喂**(流式/在线) | **攒一整段一次看**(批处理) |
| 靠什么记住过去很多帧 | **状态变量**(s_state+计数器+min/max) | **环形缓冲**(600原始样本) |
| 需缓存原始帧吗 | **不需要** | **需要** |

- **状态机**:边喂边把跨帧信息**压缩进几个状态变量**("在哪个阶段、失重几帧、冲击峰值、静止几帧"),这几个数就代表了"之前几百帧发生了什么",所以**不存原始波形**。一次跌倒序列约 400~650 帧(2~3秒),`Fall_Algo_Process` 被调几百次,**只在最后确认那帧 return 1**。
- **NN**:必须拿完整 3 秒原始波形做 DSP,所以**必须用环形缓冲原样存 600 帧**。

> 类比:状态机 = **边听故事边记要点**(听完就能下结论,不用逐字背);NN = **把整段3秒录音存下来再回放分析**。这就是为什么只有 NN 需要环形缓冲、状态机不需要。
