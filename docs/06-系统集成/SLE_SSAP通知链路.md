...
[板A - Server]                        [板B - Client]
  ...
  ↓ ~100Hz 采样（主循环 osDelay 10ms）
  滑动窗口 (256采样点 × 6轴)
  ↓ 每滑动 64 采样点（约640ms）触发一次推理
  Edge Impulse 谱分析 DSP + TFLite 神经网络
  ↓ 跌倒概率 ≥ 80% + 物理门控 + 连续2窗确认
...
> 注：早期版本此处的 `~3045ms / ~2798ms / ~5.8s / ~12s` 等数据，来自未适配 WS63 的 Edge Impulse 计时函数（开发日志已记录该函数失效并移除），且基于已废弃的旧设计（100采样×3轴），不可作为依据，已删除。

| 阶段 | 时间 | 来源 |
|------|------|------|
| MPU6050 采样周期 | ≈ 10ms/采样点（≈100Hz） | 主循环 `osDelay(10)` |
| 推理触发间隔（稳态） | ≈ 640ms | 每滑动 64 采样点 |
| 单次 `run_classifier()` 耗时 | 未可靠实测 | EI 计时函数未适配 WS63 |
| SLE Notify 单向送达 | ≈ 6–12.5ms | 连接间隔 6.25–12.5ms |

**结论**：端到端延迟由 AI 推理管线主导，无线通信仅占约 12.5ms。完整的实时性 / 可靠性 / 星闪优势量化分析见 [实时性·可靠性·星闪优势量化分析](实时性_可靠性_星闪优势量化分析.md)。
